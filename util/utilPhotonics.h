...
#ifndef PHOTONICS_FUNC_SIM_APPS_UTIL_H
#define PHOTONICS_FUNC_SIM_APPS_UTIL_H
...
bool initPhotonicsAccel(const char *configFile)
{
  if (configFile == nullptr)
  {
    // Each rank has 8 chips; Total Bank = 16; Each Bank contains 32 subarrays;
    unsigned numRanks = 2;
    unsigned numBankPerRank = 2; // 8 chips * 16 banks
  ...
  {
    PhotonicsStatus status = photonicsCreateDeviceFromConfig(PHOTONICS_FUNCTIONAL, configFile);
    if (status != PHOTONICS_OK)
    {
      std::cout << "Abort" << std::endl;
      return false;
