// File: photonicsParamsHBMDram.cpp
// PHOTONICSeval Simulator - HBM DRAM parameters
...
//! @brief  photonicsParamsHBMDram ctor (based on HBM2_4Gb_x128.ini from DRAMsim3)
photonicsParamsHBMDram::photonicsParamsHBMDram()
  : // [dram_structure]
  m_protocol("HBM"),
  m_bankgroups(4),
  m_banksPerGroup(4),
...
//! @brief  photonicsParamsHBMDram ctor with a config file
photonicsParamsHBMDram::photonicsParamsHBMDram(std::unordered_map<std::string, std::string> params)
{
  try {
    ...
    m_numDies = std::stoi(photonicsUtils::getParam(params, "num_dies"));
    ...
    m_tCKSRE = std::stoi(photonicsUtils::getParam(params, "tCKSRE"));
    ...
    m_unifiedQueue = photonicsUtils::getParam(params, "address_mapping") == "False" ? false : true;
